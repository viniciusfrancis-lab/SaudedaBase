...
def atualizacao_empilhado(n_ano_letivo: str, saveFolder: Path = SAVE_FOLDER, colunas_alvo: list[str] = COLUNAS_EMPILHADO):
    logger.info(f"Iniciando atualização EMPILHADO_MATRICULAS para o ano letivo: {n_ano_letivo}")
    ...
    logger.info("✅ EMPILHADO_MATRICULAS atualizado com sucesso!")


# ============================================================================
# FUNÇÃO 2: MATRICULAS_GERAIS
# ============================================================================

COLUNAS_MATRICULAS_GERAIS = [
    "id_aluno",
    "nm_aluno",
    "num_ano_letivo",
    "situacao_enturmacao",
    ...
    "nm_municipio",
    "nm_escola",
    "data_referencia",  # ✅ Adicionado aqui
]

def atualizacao_matriculas_gerais(n_ano_letivo: str, saveFolder: Path = SAVE_FOLDER, colunas_alvo: list[str] = COLUNAS_MATRICULAS_GERAIS):
    logger.info(f"Iniciando atualização MATRICULAS_GERAIS para o ano letivo: {n_ano_letivo}")
    ...
    # Carrega arquivo existente
    caminho_arquivo = saveFolder / 'MATRICULAS_GERAIS.parquet'
    if caminho_arquivo.exists():
        logger.info("Carregando base empilhada existente...")
        dfEmpilhado = pd.read_parquet(caminho_arquivo)
        DatasMatricula = dfEmpilhado.data_referencia.unique()
    ...
    for i, date in enumerate(DatasFaltantes, 1):
        try:
            ...
            # ✅ Adiciona data_referencia PRIMEIRO
            df['data_referencia'] = date
    ...
    logger.info("✅ MATRICULAS_GERAIS atualizado com sucesso!")
...
if __name__ == "__main__":
    logger.info("=" * 70)
    logger.info("INICIANDO ATUALIZAÇÃO DE AMBAS AS BASES")
    logger.info("=" * 70)
    
    try:
        atualizacao_empilhado(NUM_ANO_LETIVO)
    except Exception as e:
        logger.error(f"❌ Erro ao atualizar EMPILHADO_MATRICULAS: {e}")
    
    logger.info("-" * 70)
    
    try:
        atualizacao_matriculas_gerais(NUM_ANO_LETIVO)
    except Exception as e:
        logger.error(f"❌ Erro ao atualizar MATRICULAS_GERAIS: {e}")
